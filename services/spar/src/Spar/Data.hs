...
import Spar.Data.Instances (VerdictFormatRow, VerdictFormatCon, CqlScimUser(..), fromVerdictFormat, toVerdictFormat)
import Spar.Types
...
insertScimUser uid usr = retry x5 . write ins $
  params Quorum (uid, CqlScimUser usr)
  where
    ins :: PrepQuery W (UserId, CqlScimUser SparTag) ()
    ins = "INSERT INTO scim_user (id, json) VALUES (?, ?)"

getScimUser
  :: (HasCallStack, MonadClient m)
  => UserId -> m (Maybe (ScimC.User.StoredUser SparTag))
getScimUser uid = fromCqlScimUser . runIdentity <$$>
  (retry x1 . query1 sel $ params Quorum (Identity uid))
  where
    sel :: PrepQuery R (Identity UserId) (Identity (CqlScimUser SparTag))
...
getScimUsers uids = fromCqlScimUser . runIdentity <$$>
